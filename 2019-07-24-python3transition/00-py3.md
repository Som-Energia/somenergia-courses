...
da una la lista mas amplia de fixers portables sin necesidad de revisar, que no he testeado.
De hecho alguno no lo incluiria.
...
types
ws_comma
xreadlines # para mi con revisión, iterable
```
...
d.keys() + ['additional'] # Funciona en Py2 pero casca en Py3

# Solucion, aquí si, pasar a lista

list(d.items())[:2] # las dos primeras parejas
list(d.keys()) + ['additional'] # Lista explicita
```
## Alternativas Py2 eficientes {
	data-background-image="images/logo-somenergia-nobg.svg"
	data-background-size="80%"
	}
...
from future.builtins import iteritems
# iterkeys no se necesita, iterar directamente sobre el dict
```
```bash
...
Nosotros no solemos definir clases.

Si lo hemos hecho, cuidado con los métodos especiales:

`__bool__`, `__str__`, `next` (iteradores),`
