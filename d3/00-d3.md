...
- Se ejecutará como getter.
- El método retornará el valor, no `this`.
- Rompe la cadena de setters.
- El valor no tiene el siguiente getter.
...
sin estar definiendo variables locales.

```javascript
d3.select(vn.dom) // la raiz de mithril
  .select('.view') // el elemento de la vista donde insertar
  .append('circle') // retorna el elemento insertado
     .attr('cx', 100) // todos retornan el elemento
     .attr('cy', 100)
     .attr('r', 100)
     .attr('fill', 'red')
     ;
...
En vez de usar los interpoladores y los suavizados por defecto, ofrecemos una función.

```javascript
	// Llamadas equivalentes
	.style('fill', 'blue')
	.styleTween("fill", function() {
		// Aqui podriamos cambiar el interpolador
...
d3-random: Generar diversas distribuciones a
